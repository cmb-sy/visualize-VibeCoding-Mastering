...
export function DashboardPage({ days }: Props) {
  ...
  const trends = {
    ...
    subagent: isAll
      ? undefined
      : calcTrend(current.subagentUses, prev.subagentUses),
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <StatCard
          title="Skill実行数"
          value={
            isAll ? fmtNum(summary.total_skill_uses) : fmtNum(current.skillUses)
          }
          trend={trends.skill}
          accentColor="#ec4899"
        ...
        <StatCard
          ...
          value={
            isAll
              ? fmtNum(summary.total_subagent_uses)
              : fmtNum(current.subagentUses)
          }
        ...
        <StatCard
          title="MCP呼び出し"
          value={
            isAll ? fmtNum(summary.total_mcp_uses) : fmtNum(current.mcpUses)
          }
          trend={trends.mcp}
          accentColor="#06b6d4"
        ...
        <StatCard
          title="メッセージ"
          value={
            isAll ? fmtNum(summary.total_messages) : fmtNum(current.messages)
          }
      ...
      {/* 使いこなし度 */}
      <MasteryCard summary={summary} trend={isAll ? undefined : trends} />
